...
Sample.findOneAndUpdate({categoryName:reqq.body. mainCategory,"subCategories.subCategoryName": req.body. subCategory},{"$push":{ "subCategories.$[].items":{ 
  itemId : req.body.menuID,
  itemPrice :req.body.itemPrice,
  itemName :req.body.itemName
...
router.post("/delete", (req, res) => {
  ...
  mongoose.set('useFindAndModify', false);
Sample.findOneAndUpdate({categoryName: mainCategory,"subCategories.subCategoryName": req.body.subCategory},{"$pull":{ "subCategories.$[].items":{ 
  itemId : req.body.menuID
...
router.post("/subcategory", (req, res) => {
  ...
  mongoose.set('useFindAndModify', false);
Sample.findOneAndUpdate({categoryName:req.body.mainCategory }, {'$push' :{subCategories:[sub] }}, function(err, result) {
  if (err) {
    console.log(resssss)
  } else {
    res.send(result);
...
router.post("/addongrp", (req, res) => {
    ...
    mongoose.set('useFindAndModify', false);
    Sampleaddongrp.findOneAndUpdate( {res_id:"test"},{"$push":{ "AddOnGroups":[
  item
  ...
  router.post("/addon", (req, res) => {
    ...
    mongoose.set('useFindAndModify', false);
    Sampleaddon.findOneAndUpdate({ res_id:"test"},{'$push' :{Addons:[sub] }}, function(err, result) {
    if (err) {
     res.send(err);
    } else {
      console.log('hello');
  ...
  router.post("/deleteaddon", (req, res) => {
    ...
    // };
    mongoose.set('useFindAndModify', false);
  Sampleaddon.findOneAndUpdate({ res_id:"test"},{"$pull": {Addons:{ itemName: req.body. addOnName} }}, function(err, result) {
    if (err) {
     console.log(err)
    } else {
      console.log("hello")
  ...
  router.post("/deleteaddongrp", (req, res) => {
    ...
    // };
    mongoose.set('useFindAndModify', false);
  Sampleaddongrp.findOneAndUpdate({ res_id:"test"},{"$pull": {AddOnGroups:{AddOnGroupName:req.body.addOnGroupName} }}, function(err, result) {
    if (err) {
     console.log(err)
    } else {
      console.log("hello")
